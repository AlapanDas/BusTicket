...
export async function POST(request: Request, response: NextResponse) {
     try {
          ...
          try {
               ...
               if (hits.length > 0) {
                    return NextResponse.json({ message: `Bus with number "${newBus['bus-number']}" already exists.` },{status:304});
               } else {
               ...
               return NextResponse.json({ status: 'Error searching for bus', error },{status:501});
          }
     } catch (err) {
