...
function App() {
  ...
  const handleLoginClick = async (event) => {
    ...
    try {
      ...
      const response = await fetch('https://busticket-backend.onrender.com/?userId=alapan');
      console.log("Sent!!");
  ...
  const work = async () => {
    ...
    try {
      const response = await fetch(`https://busticket-backend.onrender.com/api?start=${start}&end=${end}`);
